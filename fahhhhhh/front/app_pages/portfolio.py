        ...
        for wl in watchlists:
            ...
            with st.container(border=True):
                ...
                with st.expander(""): 
                    ...
                    def toggle_all(name, stocks):
                        val = st.session_state[f"selectall_{name}"]
                        for stock in stocks:
                            st.session_state[f"check_{name}_{stock}"] = val

                    if stocks:
                        st.checkbox(
                            "Select all",
                            key=f"selectall_{name}",
                    ...
                    if st.session_state.get(f"run_brief_{name}"):
                        ...
                        if not selected:
                            ...
                            if err:
                                ...
                                failed = [r["stock"] for r in data["reports"] if r["status"] == "error"]
                                if failed:
